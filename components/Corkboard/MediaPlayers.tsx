...
const progressToStyle = (progressPercent: number): CSSProperties => ({
  ["--progress" as string]: `${progressPercent}%`,
});

const isLinkedSource = (src: string): boolean => /^https?:\/\//i.test(src);

const volumeToStyle = (value: number): CSSProperties => ({
  ["--volume-progress" as string]: `${Math.max(0, Math.min(100, value * 100))}%`,
});

const preventNav = (event: SyntheticEvent<HTMLElement>) => {
  event.stopPropagation();
  event.nativeEvent.stopImmediatePropagation();
};

const preventSliderClick = (event: SyntheticEvent<HTMLInputElement>) => {
  event.preventDefault();
  event.stopPropagation();
  event.nativeEvent.stopImmediatePropagation();
};

const preventSliderPointer = (event: SyntheticEvent<HTMLInputElement>) => {
  event.stopPropagation();
  event.nativeEvent.stopImmediatePropagation();
};
